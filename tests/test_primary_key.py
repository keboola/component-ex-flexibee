...
def test_custom_mode_uses_selected_columns():
    cfg = _cfg("rozvaha-po-uctech", primary_key_mode="custom", primary_key=["ucet", "mena"])
    records = [{"ucet": "311", "mena": "code:CZK"}]
    assert _resolve_primary_key(cfg, ["ucet", "mena"], _KEYLESS, records) == ["ucet", "mena"]


def test_custom_mode_rejects_unknown_column():
    cfg = _cfg("rozvaha-po-uctech", primary_key_mode="custom", primary_key=["nonexistent"])
    with pytest.raises(UserException, match="nonexistent"):
        _resolve_primary_key(cfg, ["ucet", "mena"], _KEYLESS, [{"ucet": "311"}])


def test_custom_mode_requires_at_least_one_column():
    with pytest.raises(UserException, match="primary_key"):
        _cfg("ucetni-denik", primary_key_mode="custom")


def test_custom_mode_accepts_comma_separated_string():
    cfg = _cfg("rozvaha-po-uctech", primary_key_mode="custom", primary_key="ucet, mena")
    assert cfg.primary_key == ["ucet", "mena"]


def test_none_mode_returns_no_primary_key():
    cfg = _cfg("faktura-vydana", primary_key_mode="none")
    records = [{"id": "1"}]
    assert _resolve_primary_key(cfg, ["id"], _STANDARD, records) == []
